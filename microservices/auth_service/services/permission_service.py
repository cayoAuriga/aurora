...
def create_permission_fn(
    uow: AbstractUnitOfWork, data: PermissionCreate
) -> PermissionRead:
    with uow:
        ...
        permission_data = PermissionRead.from_orm(permission)
...
def get_all_permissions_fn(uow: AbstractUnitOfWork) -> list[PermissionRead]:
    with uow:
        ...
        permission_list = [PermissionRead.from_orm(perm) for perm in permissions]
